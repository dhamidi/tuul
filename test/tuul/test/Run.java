...
public final class Run {
    ...
    public static void main(String[] arguments) {
        var suites = List.of(
                ...
                Named.fast("reload.JdkToolsTest", reload.JdkToolsTest::run),
                Named.integration("reload.JdkToolsIntegrationTest", reload.JdkToolsIntegrationTest::run),
                Named.fast("reload.GenerationTest", reload.GenerationTest::run),
                Named.fast("reload.ApplicationReloadTest", reload.ApplicationReloadTest::run),
                Named.fast("modules.ModuleGraphTest", modules.ModuleGraphTest::run),
                Named.fast("web.reload.ReloadHandlerTest", web.reload.ReloadHandlerTest::run),
                Named.fast("web.reload.JdkReloadHandlerTest", web.reload.JdkReloadHandlerTest::run),
                Named.integration("web.reload.HttpRevisionSourceTest", web.reload.HttpRevisionSourceTest::run),
                Named.integration("reload.RevisionCompilerModuleLayerTest", reload.RevisionCompilerModuleLayerTest::run),
                Named.integration("modules.ModuleGraphIntegrationTest", modules.ModuleGraphIntegrationTest::run),
        ...
        for (var suite : suites) if (selected(options, suite)) Check.suite(suite.name(), suite.body(), options.timeout());
    ...
    private static Options options(String[] arguments, List<Named> suites) {
        ...
        for (var at = 0; at < arguments.length; at++) {
            switch (arguments[at]) {
                case "--suite" -> { if (++at == arguments.length) usage("--suite needs a suite name"); name = arguments[at]; }
                case "--timeout" -> {
                    if (++at == arguments.length) usage("--timeout needs seconds");
                    try { timeout = Duration.ofSeconds(Long.parseLong(arguments[at])); }
                    catch (NumberFormatException invalid) { usage("--timeout needs whole seconds"); }
                }
                case "--mode" -> {
        ...
        final var selectedName = name;
        if (!selectedName.isEmpty() && suites.stream().noneMatch(suite -> suite.name().equals(selectedName))) usage("unknown suite: " + selectedName);
        if (timeout.isZero() || timeout.isNegative()) usage("--timeout must be positive");
        final var selectedMode = mode;
        return new Options(selectedName, selectedMode, timeout);
    }
    ...
    private record Named(String name, Tier tier, Check.Suite body) {
        private static Named fast(String name, Check.Suite body) { return new Named(name, Tier.FAST, body); }
        private static Named integration(String name, Check.Suite body) { return new Named(name, Tier.INTEGRATION, body); }
    }
    private enum Tier { FAST, INTEGRATION }
