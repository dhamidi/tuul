...
/// the first request. The first request fixes the configuration. The session
/// can then create concurrent requests.
///
...
public final class Session implements AutoCloseable {
    ...
    ///
    /// The session must not have created a request. This method returns this session.
    ...
    /// A closed session throws `IllegalStateException`.
    public Request request(String method, URI uri) { start(); return new Request(this, method, uri, Headers.NONE, Body.empty(), null); }

    /// Creates a GET request with an empty body. This call starts the session.
    public Request get(URI uri) { return request("GET", uri); }

    /// Creates a HEAD request with an empty body. This call starts the session.
    public Request head(URI uri) { return request("HEAD", uri); }

    /// Creates a POST request with a non-null `body`. This call starts the session.
    ...
    ///
    /// This method is idempotent. It cancels each tracked future and rejects
    /// later request creation. A response whose headers already arrived is no
    /// longer tracked. The caller must close that response.
    ///
    /// This method does not close the shared [Fetch] connection pool.
